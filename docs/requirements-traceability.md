...
| Follows | Partial Δ | `follows` table exists in `000001_init.sql` | Day 23 adds `PUT/DELETE /pets/:petId/follow` (`social/handlers.go`'s `setFollow`, `social/routes.go`) — idempotent both directions (`ON CONFLICT DO NOTHING` / plain `DELETE`, neither errors on a repeat call), block-aware 404 via the same `visibility.ForPet` the public pet profile uses, rejects following your own pet, and inserts a `follow`-type `notifications` row for the pet's owner in the same transaction as the follow insert — but only when the row is newly created, not on a repeated idempotent PUT | local-only in reel screen | `server/integration/follows_test.go` (live, 6 cases; not run this pass — local Postgres/Redis weren't up, same graceful skip every other live test shows without `docker compose up -d`) | The follow-gated visibility this table already powered (`social` query filters, `visibility.ForPet`'s private-account check) previously had no way for a user to actually become a follower — this day closes that gap. |
