...
| Follows | Partial Δ | `follows` table exists in `000001_init.sql` | Day 23 adds `PUT/DELETE /pets/:petId/follow` (`social/handlers.go`'s `setFollow`) — idempotent both directions, block-aware 404 via `visibility.ForPet`, rejects following your own pet, notifies the owner only on actual creation. Day 24 adds `GET /users/:userId/{followers,following}` (`internal/modules/profile/profile.go`) — cursor-paginated with a `total_count` computed by the exact same aggregate `GET /users/:userId` already reports (dedup'd by follower across an owner's pets for `followers`), a new shared `visibility.ForUser` (block-aware 404, private-account restriction, mirroring `ForPet`) | local-only in reel screen | `server/integration/follows_test.go` (live, 6 cases), `server/integration/followers_test.go` (live, 6 cases, including a pagination test asserting every item appears exactly once across pages and a cross-check that the list's `total_count` always equals the profile endpoint's own count) | The follow-gated visibility this table already powered (`social` query filters, `visibility.ForPet`'s private-account check) previously had no way for a user to actually become a follower, or to see who had — these two days close that gap end to end. Live-verified against real seeded data (Docker), not just the integration suite. |
